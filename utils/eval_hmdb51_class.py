...
class HMDBclassification(object):
    ...
    def evaluate(self):
        ...
        method.
        """
        num, sum = compute_video_hit_at_k(self.ground_truth,
                                          self.prediction, top_k=self.top_k)
        if self.verbose:
            print('[RESULTS] Performance on HMDB51 untrimmed video '
                   'classification task.')
            cate = open("category.txt","r")
            lines = cate.readlines()
            for class_id in range(len(num)):
                label = lines[class_id].replace("\n","")
                print('{} Accuracy@{}: {}'.format(label, self.top_k, float(num[class_id]) / float(sum[class_id])))
...
def compute_video_hit_at_k(ground_truth, prediction, top_k=3):
    ...
    for i, vid in enumerate(video_ids):
        ...
        sum[gt_label[0]] += 1
        #avg_hits_per_vid[i] = np.sum([this_pred['ratio'] if this_label in pred_label else 0 for i,this_label in enumerate(gt_label)]
        for i, this_label in enumerate(pred_label):
            if this_label in gt_label:
                num[gt_label[0]] += 1
