...
class UCFclassification(object):
    ...
    def evaluate(self):
        ...
        if self.verbose:
            #print('[RESULTS] Performance on UCF101 untrimmed video '
            #       'classification task.')
...
def compute_video_hit_at_k(ground_truth, prediction, top_k=3):
    ...
    logfile = open("log_video_all_ucf101_baseline.txt","w")
    fin = open("category_ucf101.txt","r")
    cate = fin.readlines()
    for i, vid in enumerate(video_ids):
        ...
        for this_label in gt_label:
            if this_label in pred_label:
                #logfile.write(cate[this_label].strip() + ' ' + str(vid) + ' 1\n')
                logfile.write(str(this_label) + ' ' + str(vid) + ' 1\n')
            else:
                logfile.write(cate[this_label].strip() + ' ' + str(vid) + ' 0\n')
